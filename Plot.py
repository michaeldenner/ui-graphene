...
class DraggableColorbar(object):
    """
    Class implementing an interactive colorbar
    """
...
class Plot:
    ...
    def calc_edge(self):
        ...
        """
        for i in range(len(self.Hamiltonian.lattice.orb)):
            plt.scatter(kx, E[:, i], s=4, color='k')
            plt.xticks([0, np.pi / (np.sqrt(3)), 2 * np.pi / (np.sqrt(3))], ('$\Gamma$', 'X', '$\Gamma$'))
            for j in range(len(kx)):
                plt.scatter(kx[j], E[j, i], s=8, color='r', alpha=up[j, i])
                plt.scatter(kx[j], E[j, i], s=8, color='b', alpha=low[j, i])
        """
    ...
    def calc_layer_pol(self):
        ...
        #cbar = ax1.colorbar(format='%05.2f')
        #cbar.set_label('y coordinate', rotation=270, labelpad=15)
        #cbar.set_norm(Normalize(vmin=self.Hamiltonian.lattice.orb[0, 1], vmax=self.Hamiltonian.lattice.orb[-1, 1]))
        #cbar = DraggableColorbar(cbar, img)
        #cbar.connect()
        ...
        #cbar = ax2.colorbar(format='%05.2f')
        #cbar.set_label('y coordinate', rotation=270, labelpad=15)
        #cbar.set_norm(Normalize(vmin=self.Hamiltonian.lattice.orb[0, 1], vmax=self.Hamiltonian.lattice.orb[-1, 1]))
        #cbar = DraggableColorbar(cbar, img)
        #cbar.connect()
